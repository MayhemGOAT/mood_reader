...
def _valid_vibe_mask(vibe_col: pd.Series) -> pd.Series:
    """Rows with a real vibe label (not NaN, blank, or the string 'nan')."""
    stripped = vibe_col.astype(str).str.strip()
    return vibe_col.notna() & (stripped != "") & (stripped.str.lower() != "nan")
...
def build_feature_matrix(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Extracting features"):
        ...
        try:
            ...
            # e.g. lyrics that are only section headers ("[Verse]") clean to empty text
            skipped += 1
            continue
    ...
    if skipped:
        print(
            f"build_feature_matrix: skipped {skipped} row(s) with empty/unusable lyrics",
            file=sys.stderr,
        )
...
def train_from_dataframe(feat_df: pd.DataFrame, model_dir: str | Path) -> dict:
    ...
    if "valence" in feat_df.columns:
        y_val = pd.to_numeric(feat_df["valence"], errors="coerce")
        mask = y_val.notna()
        if int(mask.sum()) >= 10:
            X_tr, X_te, y_tr, y_te = train_test_split(
                X.loc[mask], y_val.loc[mask], test_size=0.2, random_state=42
            )
    ...
    if "energy" in feat_df.columns:
        y_eng = pd.to_numeric(feat_df["energy"], errors="coerce")
        mask = y_eng.notna()
        if int(mask.sum()) >= 10:
            X_tr, X_te, y_tr, y_te = train_test_split(
                X.loc[mask], y_eng.loc[mask], test_size=0.2, random_state=42
            )
    ...
    if (vibe_col is None or int(_valid_vibe_mask(vibe_col).sum()) < 10) and val_model and eng_model:
        v = pd.to_numeric(feat_df["valence"], errors="coerce")
        e = pd.to_numeric(feat_df["energy"], errors="coerce")
        inferred = [
            vibe_from_valence_energy(float(vv), float(ee)) if pd.notna(vv) and pd.notna(ee) else ""
    ...
    if vibe_col is not None:
        ...
        X_vibe = X.loc[y_vibe.index]
        if len(y_vibe) >= 10 and y_vibe.nunique() >= 2:
