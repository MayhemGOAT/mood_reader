...
def build_feature_matrix(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Extracting features"):
        ...
        try:
            ...
            # Lyrics that reduce to nothing after cleaning (e.g. only "[Verse]")
            skipped += 1
            continue
...
def train_from_dataframe(feat_df: pd.DataFrame, model_dir: str | Path) -> dict:
    ...
    if "valence" in feat_df.columns:
        ...
        mask = y_val.notna()
        if int(mask.sum()) >= 10:
            X_val, y_val = X.loc[mask], y_val.loc[mask].astype(float)
    ...
    if "energy" in feat_df.columns:
        ...
        mask = y_eng.notna()
        if int(mask.sum()) >= 10:
            X_eng, y_eng = X.loc[mask], y_eng.loc[mask].astype(float)
    ...
    if vibe_col is not None:
        valid_vibe_mask = (
            ...
            & (vibe_col.astype(str).str.strip().str.lower() != "nan")
        )
        if int(valid_vibe_mask.sum()) >= 10:
