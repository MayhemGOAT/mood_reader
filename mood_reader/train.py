...
def build_feature_matrix(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Extracting features"):
        ...
        try:
            ...
            # Lyrics that are only section headers ("[Verse]") clean to empty text.
            skipped += 1
            continue
...
def train_from_dataframe(feat_df: pd.DataFrame, model_dir: str | Path) -> dict:
    ...
    if vibe_col is not None:
        valid_vibe_mask = (
            vibe_col.notna()
            & (vibe_col.astype(str).str.strip() != "")
            & (vibe_col.astype(str).str.lower() != "nan")
        )
        y_vibe = vibe_col.loc[valid_vibe_mask].astype(str)
        if int(valid_vibe_mask.sum()) >= 10 and y_vibe.nunique() >= 2:
