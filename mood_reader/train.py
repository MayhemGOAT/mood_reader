...
def build_feature_matrix(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Extracting features"):
        ...
        try:
            ...
            # Lyrics that reduce to nothing after cleaning (e.g. header-only
            # "[Verse]") raise ValueError; skip them instead of aborting the run.
            skipped += 1
            continue
    ...
    if skipped:
        print(f"Skipped {skipped} row(s) with empty/unusable lyrics", file=sys.stderr)
...
def train_from_dataframe(feat_df: pd.DataFrame, model_dir: str | Path) -> dict:
    ...
    if vibe_col is not None:
        ...
        valid_mask = vibe_col.notna() & (vibe_str != "") & (vibe_str.str.lower() != "nan")
        y_vibe = vibe_str[valid_mask]
        X_vibe = X.loc[valid_mask]
        # GradientBoostingClassifier needs at least two classes to fit.
