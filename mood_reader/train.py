...
def build_feature_matrix(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in tqdm(df.iterrows(), total=len(df), desc="Extracting features"):
        ...
        # Skip rows with no usable lyrics: a missing cell becomes the literal
        # string "nan", and an empty string makes feature extraction raise.
    ...
    if skipped:
        print(f"Skipped {skipped} row(s) with missing/empty lyrics", file=sys.stderr)
...
def train_from_dataframe(feat_df: pd.DataFrame, model_dir: str | Path) -> dict:
    ...
    # --- valence regressor ---
    # Only fit on rows that actually carry a valence label; otherwise sklearn
    # raises "Input y contains NaN" for partially-labeled datasets.
    ...
    if vibe_col is None or vibe_col.notna().sum() < 10:
        if val_model and eng_model:
            # Infer a weak vibe label only where both valence and energy exist;
            # rows missing either stay NaN so they are dropped before fitting.
            inferred = [
