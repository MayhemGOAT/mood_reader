...
def build_feature_matrix(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for idx, row in tqdm(df.iterrows(), total=len(df), desc="Extracting features"):
        ...
        try:
            ...
            # Header-only or otherwise content-free lyrics (e.g. "[Verse]").
            skipped += 1
            continue
    ...
    # Preserve the source-row index so callers can realign labels/metadata to the
    # rows that actually produced features.
...
def train_from_dataframe(feat_df: pd.DataFrame, model_dir: str | Path) -> dict:
    ...
    if "valence" in feat_df.columns:
        ...
        if int(val_mask.sum()) >= 10:
            X_tr, X_te, y_tr, y_te = train_test_split(
                X.loc[val_mask], y_val.loc[val_mask], test_size=0.2, random_state=42
            )
    ...
    if "energy" in feat_df.columns:
        ...
        eng_mask = y_eng.notna()
        if int(eng_mask.sum()) >= 10:
            X_tr, X_te, y_tr, y_te = train_test_split(
                X.loc[eng_mask], y_eng.loc[eng_mask], test_size=0.2, random_state=42
            )
    ...
    if valid_vibe_mask is not None and int(valid_vibe_mask.sum()) >= 10:
        y_vibe = vibe_col[valid_vibe_mask].astype(str)
