...
def train_from_dataframe(feat_df: pd.DataFrame, model_dir: str | Path) -> dict:
    ...
    if y_val_all is not None and int(y_val_all.notna().sum()) >= 10:
        mask = y_val_all.notna()
        X_tr, X_te, y_tr, y_te = train_test_split(
            X.loc[mask], y_val_all.loc[mask], test_size=0.2, random_state=42
        )
    ...
    if y_eng_all is not None and int(y_eng_all.notna().sum()) >= 10:
        mask = y_eng_all.notna()
        X_tr, X_te, y_tr, y_te = train_test_split(
            X.loc[mask], y_eng_all.loc[mask], test_size=0.2, random_state=42
        )
    ...
    if vibe_col is None or vibe_col.notna().sum() < 10:
        if val_model is not None and eng_model is not None:
            inferred = [
                ...
                if pd.notna(v) and pd.notna(e) else None
    ...
    if vibe_col is not None:
        ...
        valid_vibe_mask = vibe_col.notna() & vibe_str.ne("") & vibe_str.str.lower().ne("nan")
    else:
