...
def _normalize(text: object) -> str:
    if text is None or (isinstance(text, float) and pd.isna(text)):
        return ""
    # Fold accents to ASCII (é -> e, ñ -> n) before stripping non-alphanumerics,
    # so accented international names match their unaccented spellings across
    # datasets instead of dropping the accented letter entirely.
