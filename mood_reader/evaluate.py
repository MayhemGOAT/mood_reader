...
def train_evaluate_labeled_split(
    ...
    *,
    ...
    # source-row index, so split by that index rather than by position.
    train_feat = feat_df[feat_df.index < n_train].copy()
    test_feat = feat_df[feat_df.index >= n_train].copy()
    # Realign test_df to only the test rows that produced features so per-song
    # predictions written to test_predictions.csv stay row-aligned.
