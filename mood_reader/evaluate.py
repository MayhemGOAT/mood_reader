...
def train_evaluate_labeled_split(
    ...
    *,
    ...
    # build_feature_matrix preserves each source row's index and may drop rows
    # with unusable lyrics. Select train/test rows by their original position in
    # `combined` rather than positional slicing, which would misalign after drops.
    train_feat = feat_df[feat_df.index < n_train].copy()
    test_feat = feat_df[feat_df.index >= n_train].copy()
    # Realign test_df to exactly the test rows that survived feature extraction so
    # predictions line up with their source rows.
